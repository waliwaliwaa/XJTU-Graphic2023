...
Vector3f WhittedRenderer::cast_ray(const Ray& ray, const Scene& scene, int depth)
{
    ...
    std::optional result = trace(ray, scene);
    ...
    if (result.has_value()) {
        ...
        if (material.shininess >= 1000) {
            // 不知道ior是多少，自己手动设置的
            float kr = fresnel(ray.direction, intersection.normal, 4);
            Ray reflection;
            // reflection.origin = hitPoint + intersection.normal * EPSILON;
            reflection.direction =
                reflect(ray.direction, intersection.normal.normalized()).normalized();
